...
File is able to connect to LEAP caa 13/4/20
It is very hard for Y = 0
Make sure script is able to write to Python
Check out Arduino script with Adafruit script
'''
            ...
            if self.currenttime-self.prevtime > 0.1:                                                    # If computer time difference between times is 10ms
                ...
                for hand in frame.hands:                                                                # For each element (hand) in list of Hand objects
                   ...
                   # 2) [HAND]  Get RPY for 4th and 5th servo. Return self.ROLL and self.PITCH
                    ...
                    # Not printing for some reason. Check how to call properly again.
                    s1_Angle = self.ik2D.angle_for_armZ(self.leapX)
                 
                    # Check this if it is not printing
                    ...
                    self.ac.servo_write(self.SERVO_1, int(s1_Angle) )                                          # Done, add hard limit. Redesignate
                    ...
                    self.ac.servo_write(self.SERVO_4, int(s4_Angle) )     # Uncheck once Arduino handshake is done                                     # Deconflict servo 4
                    self.ac.servo_write(self.SERVO_5, int(s5_Angle) )     # Uncheck once Arduino handshake is done                                     # Done, add hard limit
                    self.ac.servo_write(self.SERVO_6, int(s6_Angle) )     # Uncheck once Arduino handshake is done                                     # Robot claw end effector wrt distance between the thumb and index finger
